...
w= sigmaw*randn(n,1);      % noise on system 
v= sigmav*randn(n,1);      % noise on measurement
u= square((1:n)*fu*2*pi)'; % Generate a squre signal with frequence fu

x= zeros(n,1);             % zero vectors 
y= zeros(n,1);
x(1)= sigmax0*randn+x0;    % initial values 
y(1)= sin(c*x(1)+phih)+v(1);
for i= 2:n;                % System dynamics of k+1
  x(i)= a*sin(x(i-1)+phif)+b*u(i-1)+w(i-1);
  y(i)= sin(c*x(i)+phih)+v(i);
end;
